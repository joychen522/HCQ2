...
namespace HCQ2UI_Logic.AdminController
{
    ...
    public class SysLimitController:BaseLogic
    {
        ...
        #region 2.1 保存单位代管--权限设置 + ActionResult SaveUnitLimitData(int id)
        /// <summary>
        ///  2.1 保存单位代管--权限设置
        /// </summary>
        /// <returns></returns>
        [HttpPost]
        public ActionResult SaveUnitLimitData(int id)
        {
            string menus = RequestHelper.GetStrByName("menuData");
            string reak = RequestHelper.GetStrByName("reak");//是否处理标记
            try
            {
                bool back = operateContext.bllSession.T_B01PermissRelation.SaveUserUnitData(menus, reak, id);
                //清理单位代管权限缓存
        ...
        #region 2.2 根据权限ID获取 对应单位信息 + ActionResult GetUnitTreeData(int id)
        /// <summary>
        ///  2.2 根据权限ID获取 对应单位信息
        /// </summary>
        ...
        {
            List<Dictionary<string, object>> list = operateContext.bllSession.T_B01PermissRelation.GetLimitUnitDataById(id);
