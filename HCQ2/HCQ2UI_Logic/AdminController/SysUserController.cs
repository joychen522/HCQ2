    ...
    public class SysUserController:BaseLogic
    {
        ...
        public ActionResult GetUserData()
        {
            string keyword = Helper.ToString(Request["userName"]),//用户名
                unitID = RequestHelper.GetStrByName("unitID");//单位代码
            ...
            List<UserModel> user= base.operateContext.bllSession.T_User.GetUserData(unitID,keyword, page, rows);
            TableModel tModel = new TableModel()
            {
                total = operateContext.bllSession.T_User.GetCountByData(keyword,unitID),
                rows = user
            };
        ...
        {
            List<Dictionary<string, object>> list = operateContext.bllSession.T_UserUnitRelation.GetUserUnitDataById(id);
