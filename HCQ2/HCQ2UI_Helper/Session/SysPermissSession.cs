...
namespace HCQ2UI_Helper.Session
{
    ...
    public class SysPermissSession
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static List<B01> PerminssUnitData
        {
            get
            {
                object cache = SessionHelper.GetSessionValue(CacheConstant.loginUserPerminssUnitData);
                List<B01> list = (null != cache) ? (List<B01>) cache : null;
                if (null == list)
                {
                    list= OperateContext.Current.bllSession.B01.GetUnitDataByPermiss(OperateContext.Current.Usr.user_id);
                    SessionHelper.AddSessionValue(CacheConstant.loginUserPerminssUnitData, list);
                }
                return list;
            } 
        }
        ...
        public static List<AreaModel> PerminssAreaData
        {
            get
            {
                object cache = SessionHelper.GetSessionValue(CacheConstant.loginUserPerminssAreaData);
                List<AreaModel> list = (null != cache) ? (List<AreaModel>)cache : null;
                if (null == list)
                {
                    list = OperateContext.Current.bllSession.BB_ItemPreserve.GetPermissAreaData(OperateContext.Current.Usr.user_id);
                    SessionHelper.AddSessionValue(CacheConstant.loginUserPerminssAreaData, list);
                }
                return list;
            }
        }
