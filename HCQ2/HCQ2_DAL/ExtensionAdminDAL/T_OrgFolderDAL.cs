    ...
    public partial class T_OrgFolderDAL:HCQ2_IDAL.IT_OrgFolderDAL
    {
        ...
        public List<HCQ2_Model.SelectModel.ListBoxModel> GetOrgDataByPerson()
        {
            ...
            sb.Append(@"SELECT text=(CASE WHEN ISNULL(b.UnitName,'')<>'' THEN u.user_name+'('+ISNULL(b.UnitName,'')+')' ELSE u.USER_NAME END),CAST(u.user_id AS NVARCHAR(100)) AS value FROM
                ...
                (SELECT UnitID,UnitName FROM dbo.B01) b ON o.UnitID=b.UnitID 
        ...
        public List<HCQ2_Model.SelectModel.ListBoxModel> GetFineOrgDataByPerson(int folder_id)
        {
            if (folder_id <= 0)
                return null;
            sb?.Clear();
            sb.Append(string.Format(@"SELECT text=(CASE WHEN ISNULL(b.UnitName,'')<>'' THEN u.user_name+'('+ISNULL(b.UnitName,'')+')' ELSE u.USER_NAME END),CAST(u.user_id AS NVARCHAR(100)) AS value FROM
                (SELECT user_name,user_id FROM dbo.T_User WHERE reg_from=0) u LEFT JOIN
                (SELECT UnitID,user_id FROM dbo.T_Org_User) o ON u.user_id=o.user_id LEFT JOIN
                (SELECT UnitID,UnitName FROM dbo.B01) b ON o.UnitID=b.UnitID 
