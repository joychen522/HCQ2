    ...
    public partial class T_UserBLL:IT_UserBLL
    {
        ...
        public bool AddUser(UserModel user)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(user.orgUnit))
                    DBSession.IT_Org_UserDAL.Add(new T_Org_User() {user_id = t_user.user_id, UnitID = user.orgUnit});
                //3：保存用户--角色设置
        ...
        /// <returns></returns>
        public List<UserModel> GetUserData(string unitid, string keyword,int page,int rows)
        {
            if(string.IsNullOrEmpty(unitid))
                return DBSession.IT_UserDAL.GetUserLimt(keyword, page, rows);
            List<T_User> users = DBSession.IT_UserDAL.GetUserByUnitID(unitid, keyword, page, rows);
        ...
        {
            return DBSession.IT_UserDAL.GetCountByData(keyword, unitID);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public List<HCQ2_Model.SelectModel.ListBoxModel> GetBasePersions()
        {
            List<T_User> list = Select(s => s.reg_from == 0 && string.IsNullOrEmpty(s.user_identify)).ToList();
            List<int> uList = DBSession.IBB_ServiceUserDAL.Select(s => s.user_id > 0).ToList()?.Select(s => s.user_id).ToList();
            if (null == list || list.Count <= 0)
                return null;
            List<HCQ2_Model.SelectModel.ListBoxModel> result = new List<HCQ2_Model.SelectModel.ListBoxModel>();
            foreach (var item in list)
            {
                string strTemp = "";
                if(uList.Contains(item.user_id))
                    strTemp = ":selected";
                result.Add(new HCQ2_Model.SelectModel.ListBoxModel
                {
                    text = item.user_name + "(" + item.login_name + ")",
                    value = item.user_id.ToString() + strTemp
                });
            }
            return result;
        }
